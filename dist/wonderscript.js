...
(function (pbnj) {
    ...
    (function (wonderscript) {
        ...
        Env.prototype.trace = function (opts) {
            var opts = opts || {};
            return _.vector(opts.ident || this.getIdent(), opts.source || this.getSource(), opts.line || this.getLine(), opts.column || this.getColumn());
        };
        ...
        Env.prototype.stacktrace = function () {
            ...
            while (scope) {
                if (scope.trace)
                    trace = _.conj(trace, scope.trace);
                scope = scope.parent;
            }
            return trace;
        };
        ...
        var evalCollectionLiteral = function (exp, env) {
            if (_.isMap(exp)) {
                return mori.into(mori.hashMap(), mori.map(function (xs) { return mori.vector(ws.eval(mori.nth(xs, 0), env), ws.eval(mori.nth(xs, 1), env)); }, exp));
            }
            else if (_.isVector(exp)) {
                return mori.into(mori.vector(), mori.map(function (x) { return ws.eval(x, env); }, exp));
            }
            else if (_.isSet(exp)) {
                return mori.into(mori.set(), mori.map(function (x) { return ws.eval(x, env); }, exp));
            }
            else {
        ...
        var evalVariable = function (exp, env) {
            ...
            if (mod === null)
                throw new Error(["Undefined variable: '", exp, "'"].join(''));
        ...
        var defineVariable = function (env, ident, value, meta) {
            ...
            if (isPrivate) {
                ...
                if (!mod)
                    throw new Error(["module ", ns, " is undefined"].join(''));
                if (!mod["@@SCOPE@@"]) {
        ...
        var evalProtocol = function (exp, env) {
            ...
            name = _.first(rest);
            if (!_.isSymbol(name))
                throw new Error('name should be a symbol');
            ...
            if (meta)
                meta_ = _.merge(meta_, meta);
            if (doc)
                meta_ = _.assoc(meta_, _.keyword('doc'), doc);
        ...
        var evalType = function (exp, env) {
            ...
            if (!_.isVector(fields))
                throw new Error('field list should be a vector of symbols');
            argc = _.count(fields);
            ...
            ctr.prototype.toString = function () {
                var that = this;
                var vars = _.reduce(function (s, x) { return [s, ', ', x].join(''); }, _.map(function (x) { return [x, ': ', ws.inspect(that[x])].join(''); }, names));
        ...
        Lambda.prototype.bind = function (values) {
            ...
            if (body == null) {
                ...
                if (body == null)
                    throw new Error(["wrong number of arguments for: ", this, ' got: ', args.length].join(''));
            }
            var i = 0;
        ...
        Lambda.prototype.exec = function () {
            var body = this.body;
            if (!body)
                throw new Error('the lambda must be bound to arguments first');
        ...
        var evalLoop = function (exp, env) {
            ...
            if (!_.isVector(bindings))
                throw new Error('bindings should be a vector');
        ...
        var evalCond = function (exp, env) {
            var rest = _.rest(exp);
            if (_.count(rest) % 2 !== 0)
                throw new Error(['cond requires an even number of elements ', ws.inspect(exp)].join(''));
        ...
        var evalAssignment = function (exp, env) {
            ...
            if (scope === null)
                throw new Error(["Undefined variable '", name, "'"].join(''));
        ...
        var evalPropertyAccessor = function (exp, env) {
            ...
            if (obj == null)
                throw new Error(["nil is not an object from: ", ws.inspect(_.second(exp))].join(''));
        ...
        var evalPropertyAssignment = function (exp, env) {
            var obj = ws.eval(_.second(exp), env);
            if (obj == null)
                throw new Error("nil is not an object");
            var prop = _.second(_.rest(exp));
        ...
        var evalModuleName = function (name, root) {
            ...
            if (!root)
                throw new Error('root object is required');
        ...
        var findModule = ws.findModule = function (name) {
            ...
            for (var i = 0; i < path.length; i++) {
                mod = mod[path[i]];
                if (mod == null)
                    break; //throw new Error(_.str('module ', name, ' is undefined'));
            }
            return mod;
        };
        ...
        var evalMethodApplication = function (exp, env) {
            ...
            if (obj == null)
                throw new Error(['nil is not an object from: ', ws.inspect(exp)].join(''));
        ...
        var evalTryBlock = function (exp, env) {
            ...
            if (!catchBlock && !finallyBlock)
                throw new Error("A try block should have a catch block or a finally block");
            if (catchBlock) {
                try {
                    while (expr = _.first(body)) {
                        value = ws.eval(expr, scope);
    ...
    })(wonderscript = pbnj.wonderscript || (pbnj.wonderscript = {}));
})(pbnj || (pbnj = {})); // namespace pbnj.wonderscript
//# sourceMappingURL=wonderscript.js.map